...
async function storeMsg(objName, storeMsgId, dataType, metaData, isRecved) {
    try {
        ...
        let storeGroup = { Whisper: 32, Meeting: 64, Speech: 128 }[objName] || 0;
        ...
        if (storeMsgId === TalkingMsgId && storeGroup === TalkingGroup) {
            ...
            console.log("get msg data type " + messageEntry.dataType);
            switch (Number(messageEntry.dataType)) {
                ...
                case 3: {
                    ...
                    console.log("get picture .messageEntry data is " + messageEntry.messageData + "  .  key is " + imgIdentifier);
                ...
                case 42: {
                    //greeting
                    console.log("get greeting " + messageEntry.timestamp);
                    let greetingIndex = messageEntry.messageData[0];
                    let greetingMsg = MsgTextDecoder.decode(messageEntry.messageData.slice(0));
                    ...
                    break;
                case 43: {
                    console.log("get confirmation " + messageEntry.timestamp);
                    let confirmIndex = messageEntry.messageData[0];
                    let confirmIndexMsg = MsgTextDecoder.decode(messageEntry.messageData.slice(0));
                    ...
                    button.addEventListener("click", function () {
                        ...
                        if (userInput === null) {
                            console.log("User canceled tunnel .");
                        } else {
        ...
        return new Promise((resolve, reject) => {
            indexRequest.onsuccess = () => {
                ...
                messages.forEach(message => {
                    ...
                    prefixSpan.classList.add("prefix");
                    console.log("get msg data type " + message.dataType);
                    switch (Number(message.dataType)) {
                        ...
                        case 3: {
                            ...
                            console.log("get picture .message data is " + message.messageData + "  .  key is " + imgIdentifier);
                            readBlob(3, imgIdentifier).then(blob => {
                                let imgURL = URL.createObjectURL(blob);
                                let imgElement = document.createElement("img");
                                imgElement.src = imgURL;
                        ...
                        case 42: {
                            //greeting
                            console.log("get greeting " + message.timestamp);
                            let greetingIndex = message.messageData[0];
                            ...
                            button.addEventListener("click", function () {
                                ...
                                if (userInput === null) {
                                    ...
                                    let confirmation = MsgTextEncoder.encode(userInput);
                                    let confirmationArray = new Uint8Array(2 + confirmation.length);
                                    confirmationArray[0] = 8;
                                    confirmationArray[1] = greetingIndex;
                                    confirmationArray.set(confirmation, 2);
                                    ws.send(confirmationArray);
                                    deleteNotify(message.timestamp);
                                }
                            ...
                            break;
                        case 43: {
                            console.log("get confirmation " + message.timestamp);
                            let confirmIndex = message.messageData[0];
                            let confirmIndexMsg = MsgTextDecoder.decode(message.messageData.slice(0));
                            ...
                            button.addEventListener("click", function () {
                                ...
                                if (userInput === null) {
                                    ...
                                    let tunnel = MsgTextEncoder.encode(userInput);
                                    let tunnelArray = new Uint8Array(2 + tunnel.length);
                                    tunnelArray[0] = 16;
                                    tunnelArray[1] = confirmIndex;
                                    tunnelArray.set(tunnel, 2);
                                    ws.send(tunnelArray);
                                    deleteNotify(message.timestamp);
                                }
                            });
...
function switchTab(tabId) {
    ...
    console.log("ActiveTab    " + ActiveTab + "    TalkingTab    " + TalkingTab);
...
function deliverSOS() {
    ...
    let sosArray = new Uint8Array(5 + alertMsgArrayLen);
    sosArray[0] = 2;
    sosArray[1] = document.getElementById("list1").value;
    sosArray[2] = document.getElementById("list2").value;
    sosArray[3] = document.getElementById("list3").value;
    sosArray[4] = document.getElementById("list4").value;
    sosArray.set(alertMsgArray, 5);
    ws.send(sosArray);
...
function blurEffect(event) {
    ...
    //console.log(`Touch Position Adjusted: x=${x}, y=${y}`); // Debugging log
...
function handleSettingsForm() {
    ...
    rebootButton.onclick = function () {
        ...
        if (confirm("Are you sure you want to reboot?")) {
            let rebootMsg = '{"dataType": 0,"cmdType":1}';
            ws.send(rebootMsg);
    ...
    resetButton.onclick = function () {
        ...
        if (exportFlag == true) {
            if (confirm("🚫WARNING:This action is IRREVERSIBLE.Exporting your contact first")) {
                let resetMsg = '{"dataType": 0,"cmdType":2}';
                ws.send(resetMsg);
    ...
    exportButton.onclick = function () {
        ...
        if (confirm("⚠️Export settings and contact may lead privacy leak ?")) {
            let exportMsg = '{"dataType": 0,"cmdType":3}';
            ws.send(exportMsg);
        };
...
function addMember() {
    ...
    console.log(clientIvmValue, clientIvmText);
    ...
    if (confirm("inviting " + clientIvmText + "join" + TalkingName)) {
        ...
        console.log(ivmNotify, ivmNotifyArray);
...
function sendFile(blob, byte0, byte1, byte2) {
    ...
    if (byte0 == 32) {
        ...
        console.log("store whisper file " + identifierKey);
    }
    if (byte0 == 64) {
        storeMsg("Meeting", MeetingList[byte1].msgId, byte2, identifierKey, false);
        console.log("store meeting file " + identifierKey);
    }
    ...
    reader.onload = () => {
        ...
        function sendChunk(index) {
            if (index >= totalChunks) {
                ...
                ws.send(endChunk);
                if (fileSize > 5 * 1024) {
                    //setTimeout(() => console.log("Transmission completed."), 500);
    ...
    try {
        ...
        globalEncoder = new AudioEncoder({
            output: (chunk) => {
                if (globalEncoder?.state === "configured") {
                    ...
                    chunk.copyTo(buffer);
                    chunkCount++;
                    console.log(`Encoded Opus chunk ${chunkCount}: ${buffer.length} bytes, first 10 bytes: ${buffer.slice(0, 10)}`);
                    ...
                    if (buffer.length > 50) {
                        if (!previousChunk) {
                            ...
                            console.log("Stored first chunk, waiting for second");
                            return;
                        }
                        ...
                        if (ws.readyState === WebSocket.OPEN) {
                            ws.send(combo);
                            console.log(`Sent combined chunk: ${combo.length} bytes`);
                        } else {
        ...
        processor.port.onmessage = (event) => {
            if (event.data.type === 'amplitude') {
                ...
                console.log(`Processing frame: ${event.data.data.length} samples`);
...
let speakPtt = 50;
const startSpeak = document.getElementById("startSpeak");
//document.getElementById("startSpeak").textContent = "StartVoice";
const chunkDisplay = document.getElementById("chunkSize");
let stopEncoding = null;
        ...
        if (globalEncoder && globalEncoder.state === "configured") {
            ...
            while (sampleBuffer.length >= FRAME_SIZE) {
                ...
                try {
                    ...
                    if (globalEncoder) {
                        try { globalEncoder.close(); } catch (err) {   }
...
document.getElementById("speech-tab").addEventListener("click", function () {
    ...
    document.getElementById("pic-btn").style.display = "none";
...
userList.addEventListener("click", (event) => {
    ...
    clickTimeout = setTimeout(() => {
        console.log("Single Click - Open item details");
...
userList.addEventListener("dblclick", (event) => {
    ...
    TalkingTab = ActiveTab;
    console.log("Double Click - Send WebSocket message");
    ...
    if (ActiveTab == 64) {
        ...
        if (inviteMsg.value.length == 0) {
            if (confirm("Updating the key may result in the loss of session confirmation.")) {
                let updateKeyMsg = '{"dataType": 4,"cmdType":4}';//update key
...
document.getElementById("file-btn").addEventListener("click", async () => {
    ...
    input.addEventListener("change", async (event) => {
        ...
        reader.onload = async () => {
            ...
            console.log(`File: ${file.name}, Original Size: ${file.size} bytes, Compressed Size: ${fileSize} bytes`);
        };
    });
...
document.getElementById("GPS-btn").addEventListener("click", function () {
    ...
    //showNotification(3000,currentTime);
    if (!localStorage.getItem("leafletLoaded")) {

        let script = document.createElement("script");
        script.src = "leaflet.js"; // Replace with your JS file
        document.body.appendChild(script);
        let link = document.createElement("link");
        link.rel = "stylesheet";
        link.href = "leaflet.css"; // Replace with your CSS file
    ...
    if (oldestData) {
        initMap(oldestData); // Use oldest stored location
    } else {
        showNotification(3000, "waiting received data then refresh");
    }
...
window.onload = function () {
    ...
    if (sessionStorage.getItem("reloaded")) {
        ...
        deleteMsg("Speech", 0);
        console.log("Fresh browser session.");
        ...
        if (event.data instanceof Blob) {
            ...
            switch (rcvCode) {
                ...
                case 2://alert
                    {
                        ...
                        let Type = document.getElementById("list1").options[byteArray[1]].text //
                        let Fatality = document.getElementById("list2").options[byteArray[2]].text //
                        let Struck = document.getElementById("list3").options[byteArray[3]].text //
                        let Happened = document.getElementById("list4").options[byteArray[4]].text //
                        let Note = MsgTextDecoder.decode(byteArray.slice(4));
                ...
                case 32://whisper WEB:code,listIdx,dataType,meta
                    {
                        ...
                        console.log("whisper WEB:code,", WhisperList, listIdx);
                        if (dataType == 2)
                ...
                case 64://meeting WEB:code,listIdx,dataType,meta
                    ...
                    break;
                case 128://speech WEB:code,tag,Payload_V
                    {
                        const listIdx = byteArray[1]; // Extracts the session ID                          
            ...
            console.log("Unknown data type:", event.data);
        }
    }
