  ...
  while (true) {
    ...
    if (gps.location.isValid()) {
      ...
      if ((millis() - lastPathSaving) > LocationSaveFreq * MIN_MILL) {
        ...
        PathGpsList[pathIndex].longitude = int(Latitude * GPS_INT);
            ...
            case 1:
              {
                //
                u8g2.setCursor(0, KEY);
                u8g2.print("msg map 1");
              }
              break;
            case 2:
              {
                u8g2.setCursor(0, KEY);
                u8g2.print("msg map 2");
              }
              break;
            case 3:
              {
                u8g2.setCursor(0, KEY);
                u8g2.print("msg map 3");
              }
              break;
          }
        }
...
void sosTask(void *pvParameters) {
  uint8_t requestType = 0;  //gps,HBR,RGB,beeper
  uint8_t requestMAC[6];    //rescue MAC
  uint8_t notifyInfo[DTM];  //notification
  //if SOS set phoneEnable at 250hz && centerFreq

  while (1) {
    // changing IR  and IR_FFT  for life detection when get the request
    // send geo location and path
  }
}
